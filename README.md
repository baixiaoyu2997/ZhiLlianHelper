...
* 该脚本在职位搜索页`（http://sou.zhaopin.com/）`可以选择删除无用的职位或者删除该公司所有的职位，删除按钮在发布日期下,分别为X（删除该职位）、ALL（删除该公司的所有职位），被删除的公司及职位会被记录在localStorage.list中,可以通过浏览器F12，在console中输入localStorage.list查看。
* 该脚本删除了智联首页无用的垃圾广告。
![image](http://github.com/baixiaoyu2997/ZhiLianHelper/raw/master/img/1.png)
* 如果想清除所有的记录，可以F12—console—`localStorage.clear()`
* 如果想恢复某公司，在console中先写下面代码,然后刷新页面：
```
